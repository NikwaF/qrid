...
class Autha extends CI_Controller
{
	...
	if(!empty($_SESSION['nik'])) {
        ...
        if ($this->form_validation->run() == false) { // kondisi ketika rules form login tidak terpenuhi
            $data['judul'] = 'ABSENSI UPK CERMEE  | Login Manager';
            $this->load->view('autha/_partials/header', $data);
            $this->load->view('autha/v_login');
            $this->load->view('autha/_partials/footer');
        } else {
